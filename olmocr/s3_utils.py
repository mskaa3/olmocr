...
def run_rclone_command(command: List[str]) -> subprocess.CompletedProcess:
    """Run an rclone command and return the process result."""
    try:
        result = subprocess.run(command, capture_output=True, text=True, check=True)
        return result
    except subprocess.CalledProcessError as e:
        logger.error(f"rclone command failed: {' '.join(command)}\n{e.stderr}")
        raise

def parse_s3_path(s3_path: str) -> tuple[str, str]:
    if not (s3_path.startswith("rclone://")):
        raise ValueError("Path must start with rclone://")
...
def expand_s3_glob(s3_glob: str) -> dict[str, str]:
    """Use rclone to list files matching a pattern."""
    remote, path = parse_s3_path(s3_glob)
    prefix = posixpath.dirname(path)
    pattern = posixpath.basename(path)

    command = ["rclone", "lsjson", f"{remote}:{prefix}"]
    result = run_rclone_command(command)

    files = {}
    for item in eval(result.stdout):  # Convert JSON output to Python dict
        if glob.fnmatch.fnmatch(item["Path"], pattern):
            files[f"rclone://{remote}/{item['Path']}"] = item["ModTime"]
    
    if not files:
        raise ValueError(f"No matching files found for {s3_glob}.")
    
    return files

def get_s3_bytes(s3_path: str, start_index: Optional[int] = None, end_index: Optional[int] = None) -> bytes:
    """Download a file from rclone storage as bytes."""
    remote, key = parse_s3_path(s3_path)
    command = ["rclone", "cat", f"{remote}:{key}"]
    result = run_rclone_command(command)
...
def get_s3_bytes_with_backoff(s3_path: str, max_retries: int = 8, backoff_factor: int = 2):
    """Retry downloading with exponential backoff."""
    attempt = 0
    while attempt < max_retries:
        try:
            return get_s3_bytes(s3_path)
        except Exception as e:
            wait_time = backoff_factor**attempt
            logger.warning(f"Attempt {attempt+1} failed: {e}. Retrying in {wait_time} seconds...")
            time.sleep(wait_time)
            attempt += 1
    logger.error(f"Failed to get bytes from {s3_path} after {max_retries} retries.")
    raise Exception("Download failed after retries.")

def put_s3_bytes(s3_path: str, data: bytes):
    """Upload bytes to rclone storage."""
    remote, key = parse_s3_path(s3_path)
    command = ["rclone", "rcat", f"{remote}:{key}"]
    process = subprocess.Popen(command, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
    process.communicate(input=data)
    if process.returncode != 0:
        raise RuntimeError(f"rclone upload failed: {process.stderr.read().decode()}")

def download_zstd_csv(s3_path: str):
    """Download and decompress a .zstd CSV file from rclone storage."""
    import zstandard as zstd
    try:
        compressed_data = get_s3_bytes(s3_path)
        ...
        logger.info(f"Failed to find {s3_path}. Starting fresh: {e}")
        return []

def upload_zstd_csv(s3_path: str, lines: List[str]):
    """Compress and upload a .zstd CSV file to rclone storage."""
    import zstandard as zstd
    joined_text = "\n".join(lines)
    compressor = zstd.ZstdCompressor()
    compressed = compressor.compress(joined_text.encode("utf-8"))
    put_s3_bytes(s3_path, compressed)
